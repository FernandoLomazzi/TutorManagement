...
public class AlertManager {
	...
	private static Pair<MFXGenericDialog, MFXStageDialog> createAlert(String title, String contentText,
			AlertType alertType, Pane owner) {
		MFXGenericDialog error = new MFXGenericDialog(title, contentText);
		error.setAlwaysOnTop(true);
		MFXFontIcon icon;
		MFXStageDialog errorStage = MFXGenericDialogBuilder.build(error).toStageDialogBuilder()
				.initModality(Modality.APPLICATION_MODAL).setDraggable(true).setOwnerNode(owner)
				.setScrimPriority(ScrimPriority.WINDOW).setScrimOwner(true).get();
		switch (alertType) {
		case ERROR:
	...
	public static void genericAlert(Pane owner) {
		createAlert("Alerta", "Se ha producido un error inesperado", Alert.AlertType.NONE, owner).getValue()
				.showDialog();
	}
}
